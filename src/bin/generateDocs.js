...
const trimCode = (code) => {
  ...
  const firstIndentSize = firsLineIndentation
    ? firsLineIndentation[0].length
    : 0;
  const lastIndentSize = lastLineIndentation
    ? lastLineIndentation[0].length
    : 0;

  lines = lines.map((line, index) => {
    const lineIndentSize =
      firstIndentSize !== 0 || index === 0
...
const formatCodeSnippet = (setup, ruleName) => {
  ...
  if (setup.options) {
    paragraphs.push(
      `// "jsdoc/${ruleName}": ["error"|"warn", ${JSON.stringify(setup.options).slice(1)}`,
    );
  }
...
const getAssertions = async () => {
  const assertionFiles = (
    await glob(path.resolve(dirName, "../../test/rules/assertions/*.js"))
  )
    .filter((file) => {
  ...
  const assertionCodes = await Promise.all(
    assertionFiles.map(async (filePath, idx) => {
      /**
       * @type {{
  ...
  return {
    ...
    assertions: Object.fromEntries(
      assertionNames.map((assertionName, index) => {
...
const getSomeBranch = () => {
  const gitConfig = fs
    .readFileSync(path.join(dirName, "../../.git/config"))
    .toString();
...
const generateDocs = async () => {
  const { assertionNames, assertions } = await getAssertions();
...
const generateDocsAndWriteToDisk = async () => {
  const [docContents, docPaths] = await Promise.all([
    generateDocs(),
    getDocPaths(),
  ]);
...
const assertDocsAreUpToDate = async () => {
  ...
  for (const [idx, docContent] of docContents.entries()) {
    ...
    if (!isUpToDate) {
      throw new Error(
        "Docs are not up to date, please run `npm run create-docs` to update it.",
      );
    }
  }
};
