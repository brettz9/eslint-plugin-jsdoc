      ...
      for (const method of node.body) {
        ...
        val.props[
          /** @type {import('estree').Identifier} */ (
            /** @type {import('estree').MethodDefinition} */ (method).key
          ).name
        ] = createNode();
      ...
      for (const prop of node.properties) {
        ...
        if (propVal) {
          val.props[
            ...
            (/** @type {import('estree').Property} */ (prop).key).name
          ] = propVal;
        }
      }
...
createSymbol = function (node, globals, value, scope, isGlobal) {
  ...
  switch (node.type) {
    ...
    case "MemberExpression": {
      ...
      symbol = getSymbol(
        /** @type {import('eslint').Rule.Node} */ (nde.object),
        globals,
        block,
      );
...
const mapVariables = function (node, globals, opt, isExport) {
  ...
  switch (node.type) {
    ...
    case "VariableDeclaration": {
      for (const declaration of node.declarations) {
        const isGlobal = Boolean(
          opts.initWindow && node.kind === "var" && globals.props.window,
        );
        const symbol = createSymbol(
          /** @type {import('eslint').Rule.Node} */
          (declaration.id),
    ...
    case "FunctionDeclaration": {
      /* c8 ignore next 10 */
      if (
        /** @type {import('estree').Identifier} */ (node.id).type ===
        "Identifier"
      ) {
        createSymbol(
          /** @type {import('eslint').Rule.Node} */
...
const isUncommentedExport = function (node, sourceCode, opt, settings) {
  ...
  return isNodeExported(
    node,
    /** @type {CreatedNode} */ (parseResult.globalVars),
    opt,
  );
};
