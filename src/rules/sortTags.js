...
export default iterateJsdoc(
  ({ context, jsdoc, utils }) => {
    ...
    if (firstChangedTagLine === undefined) {
      ...
      const countTagEmptyLines = (tag) => {
        return tag.source.reduce(
          (acc, { tokens: { description, end, name, tag: tg, type } }) => {
        ...
        if (ct) {
          const fixer = () => {
            ...
            for (const [
              currIdx,
              {
                tokens: { description, end, name, tag: tg, type },
              },
              ...
              if (currentTag && !tg && !type && !name && !description && !end) {
                ...
                // eslint-disable-next-line no-loop-func -- Safe
                if (
                  lastTagsOfGroup.some((lastTagOfGroup) => {
    ...
    const fix = () => {
      ...
      const initialOffset =
        /** @type {import('../iterateJsdoc.js').Integer} */ (firstLine) +
        /** @type {import('../iterateJsdoc.js').Integer} */ (
          firstChangedTagIndex
        ) +
