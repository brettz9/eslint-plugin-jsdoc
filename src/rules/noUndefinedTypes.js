...
export default iterateJsdoc(
  ({ context, node, report, settings, sourceCode, utils }) => {
    ...
    if (Object.keys(preferredTypes).length) {
      definedPreferredTypes = /** @type {string[]} */ (
        Object.values(preferredTypes)
          .map((preferredType) => {
        ...
        ? /** @type {string[]} */ (
            comments
              ...
              .flatMap((tag) => {
                ...
                const imports =
                  /** @type {import('parse-imports').Import[]} */ (
                    getImports(imprt)
                  );
                if (!imports) {
                  return null;
    ...
    const tagToParsedType = (propertyName) => {
      return (tag) => {
        try {
          const potentialType =
            tag[/** @type {"type"|"name"|"namepathOrURL"} */ (propertyName)];
          return {
            parsedType:
              mode === "permissive"
