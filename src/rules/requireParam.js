...
export default iterateJsdoc(
  ({ context, jsdoc, utils }) => {
    ...
    const findExpectedIndex = (jsdocTags, indexAtFunctionParams) => {
      const remainingRoots = functionParameterNames.slice(
        indexAtFunctionParams || 0,
      );
      const foundIndex = jsdocTags.findIndex(({ name, newAdd }) => {
        return (
          !newAdd &&
          remainingRoots.some((remainingRoot) => {
