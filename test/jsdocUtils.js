...
describe("jsdocUtils", () => {
  ...
  describe("getPreferredTagNameSimple()", () => {
    context("no preferences", () => {
      context("alias name", () => {
        it("returns the primary tag name", () => {
          expect(
            jsdocUtils.getPreferredTagNameSimple("return", "jsdoc"),
          ).to.equal("returns");
        });
        it("works with the constructor tag", () => {
          expect(
            jsdocUtils.getPreferredTagNameSimple("constructor", "jsdoc"),
          ).to.equal("class");
        });
      });
      it("works with tags that clash with prototype properties", () => {
        expect(
          jsdocUtils.getPreferredTagNameSimple("toString", "jsdoc"),
        ).to.equal("toString");
      });
      it("returns the primary tag name", () => {
        expect(
          jsdocUtils.getPreferredTagNameSimple("returns", "jsdoc"),
        ).to.equal("returns");
      });
    });
    ...
    context("with context", () => {
      it("returns the preferred tag name", () => {
        expect(
          jsdocUtils.getPreferredTagNameSimple(
            "returns",
            /** @type {BadArgument} */ ("badArg"),
          ),
        ).to.equal("returns");
      });
    });
    ...
    context("Unhandled param type", () => {
      it("should throw with an unknown param type", () => {
        expect(() => {
          ...
          "Unsupported function signature format: `AssignmentPattern`.",
        );
      });
    });
  });
  describe("mayBeUndefinedTypeTag()", () => {
    context("Missing tag", () => {
      it("should return `false` with a missing tag", () => {
        expect(jsdocUtils.mayBeUndefinedTypeTag(null, "permissive")).to.equal(
          true,
        );
      });
    });
  ...
  describe("flattenRoots()", () => {
    context("Provided root", () => {
      it("should prepend properties with provided root", () => {
        const roots =
          /** @type {import('../src/jsdocUtils.js').ParamInfo[]} */ ([
            ["data", ["last_modified"]],
            ["options", ["headers"]],
          ]);
        const expected = {
