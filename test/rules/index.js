...
const main = async () => {
  const ruleNames = JSON.parse(
    readFileSync(join(dirName, "./ruleNames.json"), "utf8"),
  );
  ...
  for (const ruleName of process.env.npm_config_rule
    ...
    let assertions;
    try {
      assertions = (await import(`./assertions/${camelCase(ruleName)}.js`))
        .default;
    } catch (error) {
    ...
    assertions.invalid = assertions.invalid.map((assertion) => {
      ...
      assertion.languageOptions = defaultsDeep(
        assertion.languageOptions,
        languageOptions,
      );
